...
| Другой ПК / телефон в той же LAN | **https://IP-СЕРВЕРА:8443** (подставьте IP сервера, например `172.22.2.102`) |

Порты `8081`/`8443` уже слушаются на `0.0.0.0` ([docker-compose.yml](docker-compose.yml)). **Проброс портов на публичный (внешний) IP не нужен** и для цехового Wi‑Fi не помогает — это выставление стенда в интернет. Адрес WSL (`vEthernet`, `192.168.x.x`) с телефона не виден.

**Доступ с Wi‑Fi**

1. Телефон/ноутбук в **том же** корпоративном Wi‑Fi (та же подсеть, что Ethernet стенда) → `https://IP-Ethernet:8443`.
2. Если Wi‑Fi изолирован (другая VLAN / гостевая сеть) → USB Wi‑Fi в ПК стенда → «Мобильный хот‑спот» Windows → заход на IP хот‑спота (часто `192.168.137.1:8443`); сертификат: `./docker/nginx/gen-dev-cert.sh ЭТОТ_IP` и `docker compose up -d nginx`.
3. Firewall на сервере: `powershell -File scripts/ensure-lan-access.ps1` (от администратора).

**USB-адаптер COM/USB должен быть подключён к тому же ПК, где открыт браузер** — Web Serial работает локально на клиенте, не на сервере.
