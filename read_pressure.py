...
def mida_fp32(data):
    """Holding/input FP32: BE bytes in each register, low word first."""
...
port = serial.Serial("COM15", 19200, timeout=1.0)
print("open", port.name)


def read(addr, fn, reg, cnt):
...
print("=== holding snapshot addr2 ===")
data, r = parse(read(2, 0x03, 0, 16))
...
print("=== input 0 x8, 5 reads ===")
for i in range(5):
    data, r = parse(read(2, 0x04, 0, 8))
    if data is None:
    ...
    for off in range(0, 12, 2):
        fp = all_fp(data[off : off + 4])
        print("  off", off, "reg", off // 2, {k: round(v, 6) if abs(v) < 1e8 else v for k, v in fp.items()})

print("=== targeted 2-reg reads ===")
for start in (1, 2, 3, 4):
    data, r = parse(read(2, 0x04, start, 2))
