/**
 * Показывает оператора, рабочее место и вход в #navAuthSlot.
 */
(function () {
    document.addEventListener('DOMContentLoaded', () => {
        ...
        async function renderOperatorBlock() {
            ...
            try {
                ...
                const ws = st && st.workstationName ? st.workstationName : st && st.workstationCode ? st.workstationCode : '';
                const order =
        ...
        async function refresh() {
            try {
                ...
                if (j.success && j.loggedIn) {
                    ...
                    slot.innerHTML = opHtml + '<a class="nav-link py-1 d-inline" href="/login.html">Админ</a>';
                }

                async function openOperatorAuth() {
                ...
                if (
                    ...
                    logoutOpBtn.addEventListener('click', () => {
                ...
                slot.innerHTML = '<a class="nav-link py-1" href="/login.html" title="API недоступен">Админ</a>';
            }
        }

        refresh();
        window.addEventListener('tm07-operator-changed', refresh);
        window.addEventListener('tm07-order-session-changed', refresh);
    });
})();
